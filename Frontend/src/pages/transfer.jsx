...
export default function Transfer() {
  ...
  const loadAccounts = async () => {
    try {
      ...
      if (data && data.length > 0) {
        ...
        toast.error('No accounts found. Please add an account first.', { id: 'empty-accounts-toast' });
      }
    } catch (e) {
      setAccounts([]);
      toast.error('No accounts found. Please add an account first.', { id: 'empty-accounts-toast' });
  ...
  const handleAddTransaction = async () => {
    ...
    try {
      ...
      toast.error(e.message || 'Failed to create transaction', { id: 'txn-error' });
    } finally {
      setLoading(false);
    }
  };
                    ...
                    <tbody>
                      {filteredTransactions.map(txn => (
                        <tr key={txn.id} className="border-b hover:bg-blue-50">
                          ...
                          <td className={`py-4 px-6 text-right font-bold ${txn.txn_type === "credit" ? "text-green-600" : "text-red-600"
                            }`}>
                  ...
                  {filteredTransactions.map(txn => (
                    <div key={txn.id} className="p-4 hover:bg-blue-50">
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <div className={`text-right font-bold text-sm ${txn.txn_type === "credit" ? "text-green-600" : "text-red-600"
                          }`}>
                          {txn.txn_type === "credit" ? "+" : "-"}₹{Number(txn.amount).toFixed(2)}
                        </div>
                      </div>
            ...
            <div className="space-y-3 sm:space-y-4">
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-2">
                <button
                  ...
                  disabled={loading}
                  className={`flex-1 bg-blue-600 hover:bg-blue-700 text-white py-2.5 sm:py-3 rounded-lg font-semibold text-sm sm:text-base ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
