...
import { Home, CreditCard, ReceiptIndianRupee, Gift, TrendingUp, ArrowLeftCircle, ArrowRightCircle, Plus, ArrowUpRight, ArrowDownRight, RefreshCcw, User, Settings, LogOut, PieChart, IndianRupee, Mail, Phone, MapPin, Lock, Bell, Globe, ImageOff, BarChart3 } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const MenuItem = ({ icon: Icon, label, page }) => (
    <button
      ...
      className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all ${activePage === page ? 'bg-blue-600 text-white' : 'text-gray-700 hover:bg-gray-100'
        }`}
    >
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-100 ">
      ...
      <nav className="flex items-center gap-3 px-6 py-4 overflow-x-auto scrollbar-hide md:hidden">
        ...
        <MenuItem icon={BarChart3} label="Insights" page="insights" />
        <MenuItem icon={Bell} label="Alerts" page="alerts" />
        <MenuItem icon={Gift} label="Rewards" page="rewards" />
        <MenuItem icon={Globe} label="Currency" page="currency" />
      ...
      <div className="flex min-h-screen">
        <aside className={`${sidebarOpen ? 'w-64' : 'w-20'} relative hidden md:block bg-white/60 backdrop-blur-xl shadow-xl border-r border-white/40 transition-all`}>
          <div className="p-6">
            <button onClick={() => setSidebarOpen(!sidebarOpen)} className="absolute -right-3 top-10 z-50 
              bg-blue-700 text-white shadow-lg rounded-full hover:scale-105 transition">
              {sidebarOpen ? (
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="p-10">
            ...
            {activePage === 'insights' && <InsightsDashboard />}
